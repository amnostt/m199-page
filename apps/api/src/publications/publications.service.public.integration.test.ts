...
integration("public publications PostgreSQL boundary", () => {
  ...
  beforeAll(async () => {
    ...
    module = await Test.createTestingModule({
      imports: [ConfigModule.forRoot({ isGlobal: true, ignoreEnvFile: true }), DbModule, PublicationsModule],
    }).compile();
    await module.init();
  ...
  it("returns only PUBLISHED rows in stable chronological pages", async () => {
    ...
    expect(ids.every((slug) => published.some((item) => item.slug === slug))).toBe(true);
    expect(ids.some((slug) => drafts.some((item) => item.slug === slug))).toBe(false);
    expect(first.items.every((item: { publishedAt: string }) => item.publishedAt)).toBe(true);
