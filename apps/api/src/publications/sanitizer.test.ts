/**
 * Publication sanitizer tests (rich-content-sanitization spec).
 *
...
describe("sanitizePublicationContent", () => {
  ...
  it("preserves allowed block tags (p, h2, h3, blockquote)", () => {
    ...
    const result = sanitizePublicationContent(input);
  ...
  it("preserves https:// href", () => {
    const result = sanitizePublicationContent(
  ...
  it("returns empty string for empty input", () => {
    expect(sanitizePublicationContent("")).toBe("");
  });
