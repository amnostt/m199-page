...
async function visualEvidence(page: Page) {
  return page.getByTestId("hero-visual").evaluate((visual) => {
    ...
    if (!copy || !image) throw new Error("Hero composition is incomplete");
...
test("renders the desktop hero as a clipped two-column composition", async ({
  page,
}) => {
...
test("renders the compact hero stacked with its image visible", async ({
  page,
}) => {
