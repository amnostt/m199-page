...
| Responsable                     | Mantener el contenido público                  | Administra la landing, publicaciones, salidas y responsables mediante una sesión protegida.               |
...
- Experiencia pública para landing, publicaciones, salidas, likes anónimos y versículo configurado.
- Administración autenticada de landing, publicaciones, salidas y responsables.
...
| Sitio público            | Parcial: landing, publicaciones y salidas son páginas Astro explícitas; la landing muestra el versículo configurado y las salidas admiten likes. | Verificar identidad de marca, responsive, accesibilidad, estados y SEO. |
| Administración editorial | Parcial: gestiona landing, héroe, publicaciones, salidas y responsables.                                                                            | Completar edición de responsables y desasociación de archivos.             |
...
| 3     | Desasociar archivos de salidas y héroe | La interfaz permite cargar o reemplazar, no quitar la asociación existente.                                                                               | El cliente puede enviar una intención explícita de desasociación sin borrar globalmente el `FileAsset`.            |
| 4     | Endurecer consultas y fechas           | Paginación, límites y algunas fechas conservan contratos ambiguos.                                                                                        | DTOs y servicios validan rangos, formatos y límites consistentes.                                                  |
| 5     | Definir el contrato HTTP de despliegue | CORS, origen y proxy confiable dependen de una topología aún no acordada.                                                                                 | La topología determina orígenes, proxy, cookies y errores seguros, con pruebas pertinentes.                        |
| 6     | Completar la experiencia pública       | Existe un sistema visual consolidado, pero su aplicación integral no está verificada contra [`brand.md`](./brand.md). El seed usa contenido demostrativo. | Los recorridos públicos aplican la identidad acordada y superan revisión responsive, accesibilidad, estados y SEO. |
| 7     | Preparar producción                    | Faltan build/start de API, persistencia, health dependiente, copias y retiro de `POST /echo`.                                                             | El sistema se despliega, monitorea, respalda, restaura y no expone scaffolding.                                    |
| 8     | Automatizar la entrega                 | Las verificaciones existen como comandos locales.                                                                                                         | CI/CD y E2E ejecutan el recorrido crítico y conservan evidencia antes de una release.                              |
